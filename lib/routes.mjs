...
export function buildRouteTargets(snapshot, config = loadRouteProbesConfig()) {
  ...
  for (const probe of config.gameProbes || []) {
    const key = probe.hostname;
    if (seen.has(key)) continue;
    seen.add(key);
    targets.push({
  ...
  const maxTotal = config.maxTargetsPerRun || 16;
  const maxLive = config.maxLiveTargetsPerRun || 3;
  return [...targets, ...live.slice(0, maxLive)].slice(0, maxTotal);
}

export function buildRouteEnforcement(routeData, config = loadRouteProbesConfig(), thresholds = null) {
  if (!routeData?.routes?.length) {
    return {
      ...
      thresholds: thresholds || config.enforcement || {},
    };
  }

  const rules = { ...(config.enforcement || {}), ...(thresholds || {}) };
  ...
  return {
    ...
    thresholds: {
      slowPathDeltaMs: pathDelta,
      slowRegionDeltaMs: regionDelta,
      source: rules.source || "config",
    },
    stats: {
