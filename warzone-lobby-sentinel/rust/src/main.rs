...
async fn main() {
    ...
    let fw = FirewallaClient::from_env().expect("Firewalla client config");
...
async fn run_poll_loop(state: Arc<AppState>, interval: f64, idle_interval: f64) {
    ...
    *state.source.lock() = if ok {
        format!("firewalla:{api}")
    } else {
        "unconfigured".into()
    };
    tracing::info!(
        "Autonomous Xbox mode — polling Firewalla {api} every {interval}s active / {idle_interval}s idle (Rust + 32D fold)"
    );
    if guard().release_now(&state.fw).await {
        tracing::info!("[network-guard] startup sync — cleared stale Firewalla defense rules");
    }
    ...
    loop {
        ...
        match state.fw.fetch_snapshot().await {
            Ok(mut raw) => {
                ...
                *state.polls.lock() += 1;
                *state.source.lock() = format!("firewalla:{api}");
