use crate::enrich::{classify_host, phase_code, role_index};
    ...
    if let Some(arr) = snapshot.pointer("/connections/items").and_then(|v| v.as_array()) {
        for item in arr {
            ...
            let rid = item
                ...
                .and_then(|v| v.as_str())
                .map(|s| s.to_string())
                .unwrap_or_else(|| classify_host(host));
        ...
        if let Some(flows) = snapshot.get(key).and_then(|v| v.as_array()) {
            for flow in flows {
                let host = flow
                    ...
                    .and_then(|v| v.as_str())
                    .unwrap_or("");
                if host.is_empty() || host.replace('.', "").chars().all(|c| c.is_ascii_digit()) {
                    continue;
                }
                let lat = flow.get("latencyMs").and_then(|v| v.as_f64());
                items.push((classify_host(host), lat));
            }
        }
    }
    items
