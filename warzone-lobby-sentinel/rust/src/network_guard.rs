...
impl NetworkGuard {
    ...
    pub async fn evaluate(&self, fw: &FirewallaClient, risk: &SessionRisk, snapshot: &Value) -> Value {
        ...
        if target_flood != FloodMode::Off {
            let need = flood != target_flood
                ...
                || kick_spike;
            if need {
                if let Ok(msg) = self.apply_flood(fw, target_flood, kick_spike).await {
        ...
        json!({
            ...
            "flood_guard": {
                ...
                "description": match flood {
                    FloodMode::Shield => "Tiny-packet shield (≤79B dropped) — normal game traffic unchanged",
            ...
            "mitigation": if in_match && flood == FloodMode::Shield {
                ...
                mitigation_summary(buffer, flood, kick_spike)
            },
            "packet_shield": {
                "active": flood == FloodMode::Shield,
                "tiny_max_bytes": 79,
                "description": "Drops inbound UDP/TCP ≤79 bytes (kick probes); normal Warzone packets pass",
            },
    ...
    async fn apply_flood(&self, fw: &FirewallaClient, mode: FloodMode, kick_spike: bool) -> Result<String, String> {
        match mode {
            FloodMode::Shield => {
                let level = if kick_spike { "strict" } else { "normal" };
                fw.run_script("gaming-packet-shield.sh", &["shield", level], true)
                    .await
    ...
    pub fn current_status(&self) -> Value {
        ...
        json!({
            ...
            "mitigation": if paused {
                ...
                mitigation_summary(buffer, flood, false)
            },
        })
    }
...
fn mitigation_summary(buffer: BufferMode, flood: FloodMode, kick_spike: bool) -> String {
...
fn target_in_match_shield(risk: &SessionRisk, snapshot: &Value, kick_spike: bool) -> FloodMode {
    if kick_spike || has_attack_signal(risk) {
